...
def open_editor(path: Path) -> None:
    ...
    _ = subprocess.run([editor, str(path)], check=False)  # nosec B603
    ...
    *,
    ...
    with txt_path.open("w", encoding="utf-8") as file:
        ...
        for group_name, items in grouped.items():
            ...
            _ = file.write(f"# GROUP: {smart_group_title(host, group)}\n")
            _ = file.write("# ----------------------------------------\n")
        ...
        if blocked_items:
            _ = file.write("# BLOCKED CANDIDATES - disabled by default\n")
            _ = file.write("# Remove '# ' from a URL line if you want to include it.\n")
            _ = file.write(
                "# -------------------------------------------------------\n"
            )

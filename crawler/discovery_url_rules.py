...
def segment_declares_region(value: str) -> bool:
    """Return True only when the complete segment is a region or locale token."""

    normalized = value.strip().lower().replace("_", "-")
    if not normalized:
        return False

    region_tokens = EXTRA_REGION_ALIASES.union(ISO_3166_REGION_CODES)

    if normalized in region_tokens:
        return True

    locale_match = re.fullmatch(r"([a-z]{2})-([a-z]{2})", normalized)
    if locale_match is None:
        return False

    language_or_region, region = locale_match.groups()
    return language_or_region in region_tokens or region in ISO_3166_REGION_CODES

...
class EnglishPageDetector:
    ...
    def detect_from_url(
        ...
        if hostname_parts:
            subdomain = hostname_parts[0].replace("_", "-")

            if subdomain in ENGLISH_LANGUAGE_CODES:
                return LanguageDecision(
                    is_english=True,
            ...
            if subdomain in NON_ENGLISH_LANGUAGE_CODES:
                return LanguageDecision(
                    is_english=False,
                    language_code=subdomain,
                    source="url-subdomain",
        ...
        for part in path_parts[:2]:
            if part in ENGLISH_LANGUAGE_CODES:
                return LanguageDecision(
                    is_english=True,
                    language_code=part,
                    source="url-path",
            ...
            if part in NON_ENGLISH_LANGUAGE_CODES:
                return LanguageDecision(
                    is_english=False,
                    language_code=part,
                    source="url-path",
    ...
    def is_explicitly_non_english_url(url: str) -> bool:
        ...
        return any(part in NON_ENGLISH_LANGUAGE_CODES for part in path_parts[:2])
